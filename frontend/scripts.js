const GPTResearcher = (() => {
    ...
    const listenToSockEvents = () => {
      ...
      const ws_uri = `${protocol === 'https:' ? 'wss:' : 'ws:'}//${host}${basePath}ws`;
      ...
      socket.onerror = (event) => {
        ...
        addAgentResponse({ output: '⚠️ WebSocket connection error. Check the browser console for details.' });
        updateState('error');
      };

      socket.onclose = (event) => {
        if (!event.wasClean) {
          addAgentResponse({ output: '⚠️ WebSocket closed unexpectedly. The server may have failed.' });
          updateState('error');
        }
      };
